...
function TradeCard({ trade, formatDate, onDelete, onEdit, onShare }: TradeCardProps) {
  ...
  return (
    <div className="rounded-[20px] border border-border bg-card p-4 space-y-4 transition-colors hover:border-border/80">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <p className="font-bold text-foreground text-base truncate">{trade.symbol}</p>
            <span
              className={cn(
                "text-[11px] font-bold px-2 py-1 rounded-lg inline-flex items-center gap-1 mt-1",
                trade.direction === "Long" ? "bg-profit-tint text-profit" : "bg-loss-tint text-loss",
              )}
            >
        ...
        <p className={cn("font-black text-lg shrink-0", isProfit ? "text-profit" : "text-loss")}>
      ...
      <div className="flex items-center justify-end gap-3 pt-1">
        ...
        {onDelete && (
          <button
            ...
            className="touch-target flex items-center justify-center text-loss hover:brightness-125 transition-all"
            aria-label="Delete"
          >
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-2 text-[12px]">
      ...
      <span className="text-foreground/80 font-semibold text-right truncate">{value}</span>
    </div>
  );
}
